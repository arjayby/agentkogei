...
12. As a prospective Builder, I want to understand how each pack may be used and shared before Installation, so that I know its terms.
...
35. As a Builder, I want the CLI to show the Pack Release and the file it will write before Installation, so that I can make an informed decision.
...
51. As a Builder, I want the installed Design Contract to be one self-contained file of design direction, so that I can see exactly what governs agent work by reading it.
...
54. As a subscribed Builder, I want premium retrieval to record entitlement without creating any Project identifier, so that access is recorded without disclosing my Project.
55. As a collaborator, I want to use an Installed Premium Design Pack in that Project, so that collaboration does not require every contributor to subscribe.
56. As a Builder, I want a genuine end-product Project containing an Installed Premium Design Pack to be public, so that open-source products remain possible.
57. As a Builder, I want Premium Access expiration to leave existing Installed Packs usable in their Projects, so that subscription expiry does not break completed or ongoing work.
...
59. As a Builder whose payment was refunded or reversed, I want the resulting access state stated clearly, so that I understand what ends even though local files are not remotely deleted.
60. As a Builder, I want the Terms to prohibit extraction, resale, republishing, and reuse of a Premium Design Pack in another Project, so that collaboration permission does not become redistribution permission.
...
65. As a pack author, I want each Pack Release validated from its Design Contract and internal Pack Evaluation record, so that publication checks a release without stamping metadata into the delivered file.
...
67. As an AgentKogei publisher, I want every Design Contract to be original or rightfully used content, verified at publication, so that Published Packs can be distributed lawfully.
68. As an AgentKogei publisher, I want malformed or unsafe releases blocked from publication, so that the Official Catalog remains trustworthy.
...
- The publication validator will reject invalid Design Contract structure, invalid semantic versions, missing required direction, and unsafe or executable content.
...
- The Official Catalog exposes public metadata, Pack Previews, release history, compatibility, and evaluation summaries. Open Design Contracts are served publicly as raw Markdown. Premium Design Contracts require a valid Pack Credential and active Premium Access before returning any gated bytes.
...
- The CLI performs complete retrieval and validation before mutating the Project. It then presents the release identity and the file it will write. No writes occur until the Builder explicitly confirms or passes `--yes`.
...
- A successful `add` writes the exact Design Contract for the resolved Pack Release. The file is bare design direction with no runtime dependency on AgentKogei; no separate record, snapshot directory, or checksum is created.
...
- Open `add` contacts only the Official Catalog and requires no AgentKogei identity. Premium `add` uses a Pack Credential and records entitlement without sending or creating a Project name, path, Git remote, or other Project identifier.
...
- A premium Design Contract retrieved during active Premium Access is a file the Builder keeps and may use within that Project; all collaborators may use it there, including when the Project is a genuine public end product. Keeping the file does not authorize extraction, resale, republishing, reuse in another Project, credential sharing, or continued catalog retrieval; those limits live in the website Terms.
- Premium Access expiration does not alter or delete the installed Design Contract and does not add runtime checks or DRM. A refund or payment reversal ends the affected Premium Access, but the service still does not remotely modify a Project.
- The database will extend the existing authentication model with the records required by device authorization, revocable Pack Credentials, entitlement and Installation-event records, and idempotent billing-event or entitlement state. It must not store Project names, paths, remotes, contents, prompts, generated UI, dependency inventories, or any Project identifier.
...
- The web application, CLI, Design Contract specification, and validators are released under MIT. Open Design Pack content carries no separate license: it is free to install and use in any Project, with the use grant stated on the website Terms page. Premium Design Packs stay private and access-gated, with redistribution limits stated on the website Terms page.
- Every Design Contract's content must be original or rightfully used, verified by human review before publication and recorded in the internal Pack Evaluation record rather than inside the delivered file. AI-assisted production requires human originality, accessibility, and rights review. Pack marketing may reference broad movements but must not claim to replicate a living designer, company, or recognizable commercial product.
...
- The commercial terms — the website Terms (open-pack use grant and premium redistribution limits), subscription disclosures, public-Project permissions, and no-voluntary-refund wording — are published on the website Terms and Privacy pages.
...
- The website Terms and checkout disclosures are published on the Terms and Privacy pages. Production checkout runs whenever Polar production credentials and a Premium Access product are configured.
