...
Promotion copies the approved proposal bytes into `packages/design-systems/releases/<identity>/<version>`, rebuilds generated Official Catalog artifacts, proves discovery, and reruns `launch:verify`. A successful promotion is ready to deploy but is not live.
...
Protocol `1.0` uses public Markdown routes at `/contracts/<identity>` and `/contracts/<identity>/<version>` with `x-agentkogei-design-system` and `x-agentkogei-design-system-release` headers. If verification reports another protocol or the packaged CLI cannot use these routes, stop and request a separate package release decision. Do not publish npm from this workflow.
