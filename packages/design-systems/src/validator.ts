...
export type DesignSystemValidationOptions = {
...
const requiredScreens = designSystemPreviewSurfaces;
...
function validateReleaseRules(record: DesignSystemEvaluationRecord) {
	const errors: string[] = [];
...
/**
 * A Design System Release publishes one document and the evidence Design System Evaluation rests
...
async function validateReleaseContents(
	...
	record: DesignSystemEvaluationRecord,
) {
	const errors: string[] = [];
...
/**
 * What a Design Contract may present as a fenced block. Everything a Design
 * System Release declares is a definition an implementation reads: semantic tokens
...
/**
 * A Design System Release is immutable, so republishing the same version must reproduce
...
async function validateAgainstPublishedRelease(
	...
	try {
		const publishedContents = await readFile(
			path.join(publishedReleaseDirectory, designSystemEvaluationFileName),
			"utf8",
		);
		const published = designSystemEvaluationRecordSchema.parse(
...
/**
 * Design System Evaluation as the Official Catalog runs it before a Design System Release becomes
 * a Published Design System. It is internal product tooling for first-party releases
 ...
 */
export async function validateDesignSystemRelease(
	rootDirectory: string,
	options: DesignSystemValidationOptions = {},
): Promise<DesignSystemValidationResult> {
	const errors: string[] = [];
	...
	try {
		recordContents = await readFile(
			path.join(rootDirectory, designSystemEvaluationFileName),
			"utf8",
		);
		...
		return {
			ok: false,
			errors: ["Design System Evaluation record is missing or invalid JSON"],
		};
	}

	const parsed = designSystemEvaluationRecordSchema.safeParse(source);
	if (!parsed.success) {
		return {
			ok: false,
