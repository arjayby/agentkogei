...
import { designSystemIdentityPattern } from "./design-system-identity";
...
export async function retrieveDesignContract(input: {
	...
	if (!designSystemIdentityPattern.test(input.identity)) {
		throw new Error("invalid Design System identity");
	}
	if (
		input.version !== undefined &&
		!designSystemReleaseVersionSchema.safeParse(input.version).success
	) {
