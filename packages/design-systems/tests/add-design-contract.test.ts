...
describe("agentkogei add", () => {
	...
	test("installs the current Design System Release as one root Design Contract", async () => {
		const project = await temporaryProject();
	...
	test("points agents at the Design Contract without disturbing their instructions", async () => {
		...
		expect(afterFirst).toContain("<!-- agentkogei:design-system:start -->");
		expect(afterFirst).toContain("`DESIGN.md`");
		expect(afterSecond).toBe(afterFirst);
		expect(afterSecond?.match(/agentkogei:design-system:start/g)).toHaveLength(
			1,
		);
	});
