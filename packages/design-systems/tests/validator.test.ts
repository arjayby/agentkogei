...
async function runValidator(
	...
	return JSON.parse(output) as DesignSystemValidationResult;
}
...
async function copyFoundationFixture() {
	const rootDirectory = await mkdtemp(
		path.join(tmpdir(), "agentkogei-design-system-"),
	);
...
async function readEvaluationRecord(rootDirectory: string) {
	return JSON.parse(
		await readFile(
			path.join(rootDirectory, "design-system-evaluation.json"),
			"utf8",
		),
...
describe("Design System Release publication validation", () => {
	...
	for (const designSystem of publishedDesignSystems) {
		for (const version of designSystem.versions) {
			test(`accepts the published ${designSystem.id} Design System Release ${version}`, async () => {
				const releaseDirectory = designSystem.directoryFor(version);
	...
	const invalidRecordCases = [
		...
		},
		{
			...
			mutate(record: Record<string, unknown>) {
				record.designSystem = "Foundation\u001b";
			},
	...
	test("rejects an evaluation evidence path that escapes the Design System Release", async () => {
		const errors = await evaluateMutatedRelease((record) => {
	...
	test("rejects a symlinked Design Contract", async () => {
		...
		await writeFile(outsideFile, "outside the Design System Release\n");
	...
	test("accepts an unchanged published Design System Release", async () => {
		const result = await runValidator(
			foundationReleaseDirectory,
			foundationReleaseDirectory,
		);
